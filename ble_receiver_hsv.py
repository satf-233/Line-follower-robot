...
# -*- coding: utf-8 -*-
"""
ble_receiver_hsv.py —— 电脑端 BLE 接收脚本
...
用法:
    python ble_receiver_hsv.py                    # 自动扫描并连接 ESP32S3-CAM
    python ble_receiver_hsv.py --addr XX:XX:...   # 按 MAC 地址直连（更快）
    python ble_receiver_hsv.py --save frame.jpg   # 额外保存第一帧到文件
    python ble_receiver_hsv.py --no-mtu           # 禁用 MTU 协商（仅调试用）
...
#--------------------------------------------------------------------------------------------------
        ...
        in_range = cv2.inRange(hsv, lower, upper)   #根据阈值创建掩码：范围内=255(白)
